/* eslint-disable */
// noinspection JSUnusedGlobalSymbols,JSUnresolvedVariable,JSUnusedLocalSymbols,JSUnusedAssignment,SpellCheckingInspection,JSCheckFunctionSignatures,JSPrimitiveTypeWrapperUsage,CommaExpressionJS,RegExpSimplifiable,RegExpRedundantEscape

const  evalReplacer = (code) => {
  ...
  function handleStartTag(match) {
    ...
    for (let i = 0; i < l; i++) {
      ...
      attrs[i] = {
        ...
        // 判断当前的属性是否是直接写的属性名,例如<a disabled/>中的disabled
        singleProp: args[3] === undefined && args[4] === undefined && args[5]===undefined
      }
...
if (typeof Set !== 'undefined' && isNative(Set)) {
  ...
  _Set = /*@__PURE__*/(function () {  // eslint-disable-line no-unused-vars
    function Set() {
...
tip = function (msg, vm) {  // eslint-disable-line no-unused-vars
  if (hasConsole && (!config.silent)) {
...
const he = createCommonjsModule(function (module, exports) {
  (function (root) {
    ...
    const decode = function (html, options) {
      ...
      return html.replace(regexDecode, function ($0, $1, $2, $3, $4, $5, $6, $7, $8) { // eslint-disable-line no-unused-vars
        let codePoint
        let semicolon
        let decDigits
        let hexDigits
...
function processIf(el) {
  ...
  if (typeof exp !== 'undefined' && typeof exp !== 'null') {
    ...
    if (typeof elseif !== 'undefined' && typeof elseif !== 'null') {
      el.elseif = elseif
    }
  }
}
...
function processIfConditions(el, parent) {
  ...
  if (prev && ((typeof prev.if !== 'undefined' && typeof prev.if !== 'null') || (typeof prev.elseif !== 'undefined' && typeof prev.elseif !== 'null'))) {
...
function addIfCondition(el, condition) { // eslint-disable-line no-unused-vars
  // if (!el.ifConditions) {
  //   el.ifConditions = []
  // }
...
function processSlotContent(el) {
  ...
  } else if ((slotScope = getAndRemoveAttr(el, 'slot-scope')) && el.tag!=='ta-list-item') {
    /* istanbul ignore if */
  ...
  if (slotTarget && el.tag!=='ta-list-item') {
...
function makeAttrsMap(attrs) {
  ...
  for (let i = 0; i < attrs.length; i++) {
    ...
    if (map[name] && !isIE && !isEdge) {
      warn$1('duplicate attribute: ' + name, attrs[i])
    }
    const singleProp = attrs[i].singleProp
    // 针对popover组件的v-popover指令的处理
    if (singleProp && name.includes('v-popover')){
      map[name] = 'true'
    } else if (singleProp && !name.includes('#')) {
      map[name] = true
    } else {
      map[name] = attrs[i].value
    }

    // v-bind，v-on指令相关属性
    // 属性值带=>时,
    // 不在属性值外加"",避免箭头函数被识别成字符串，props类型验证证错
    if (attrs[i].value.includes('=>') && !attrs[i].name?.includes('@') && !attrs[i].name?.includes(':')) {
      try {
        evalReplacer(`return \"${attrs[i].value}\"`)
        map[name] = `\"${attrs[i].value}\"`
      } catch {
        map[name] = String(attrs[i].value)
      }
    }
    if (attrs[i].value === 'true' || attrs[i].value === 'false') {
      map[name] = evalReplacer(`return ${attrs[i].value}`)
    }
  }
  return map
}
...
function walk(node, isRoot) { // eslint-disable-line no-unused-vars
